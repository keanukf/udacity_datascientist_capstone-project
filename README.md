...
Figure B.2
We can now also plot some scatter plots with our target variable and different numerical features, to better understand those relationship on a visual perspective (Fig. B.2). The y-axis is set between 0 and 35 for visual purposes, since we have some great outliers that make it hard to see what most of the scatter plot looks like. This is done with plotly. Looking at those plots only income and number of member days seem to be able to explain variance in spendings.
...
Figure B.14
When fitting a ridge regressor (not normalized) on our training data and plotting the resulting R-squared for different alphas we can see how our score varies between different alphas (Fig. B.14). Here we can visually see that we get best score for alpha being 14.69.
There is also another type of regularized regression thats called Lasso Regression. It can be used to select important features of a data set, because it tends to shrink coefficients of low import features to be exactly 0 (as we’ve done manually above within implementation part).
...
Regarding business decisions there are different approaches to work with this information. One way might be to focus on this high spending group and send more or even further individualized offers. Another option is to focus on the low spending group, as the high spending group already performs well considering revenue. There are possibly ways to get a better engagement of this group of people with specialized offers. The options vary between quick wins and large leverage.
What offers seem to work best regarding customer spendings?
Here we learned that BOGOs with reward and difficulty 5 and 10 seem to influence customer spendings the most. Informational offers seem to have the least impact on average customer spendings.
Regarding business decisions a conclusion could be to focus more on BOGOs in order to improve the revenue. Discount offers seem to work in general, but the data might be an indication to play around with reward and difficulty numbers. Some discount offers seem to work better than others.
How can customers be characterized regarding spending characteristics?
...
The impact of informational offers could be further examined.
Other machine learning models like Decision Trees or Random Forests could be used. All the used regression models were linear models. Maybe some important relationships can be explained better with non-linear methods. For the Decision Tree a first idea of how this might be done are given in the Jupyter Notebook of this project on GitHub.
Hyperparameters for Ridge and Lasso Regression could be determined more exactly through cross-validation, since we just estimated right alpha coefficients so far.
Then customer behavior could be examined closer, because maybe there are latent factors like buying habits, that influence customer spendings.
